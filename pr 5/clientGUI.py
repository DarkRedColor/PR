...
def sendMsg():
	global log
	message = getMessage()
	sucket.send(bytes(message, "utf-8"))
	hackOfTheCentury = bytes(message, "utf-8")
	hackOfTheCentury = str(hackOfTheCentury, 'utf-8')
	log.append(hackOfTheCentury)

def refresh():
	global log
	data = sucket.recv(1024)
	if not data:
		sys.exit()
	data = str(data, 'utf-8')
	print(log)
	for iten in range(len(log)):
		print(log[iten])
		if log[iten] in data:
			data = data.replace(log[iten], '')
	textBox.insert(INSERT, data + '\n')

def initClient(address):
	sucket.connect((address, 10000))

def callback():
	root.quit()

def connect(route):
	client = initClient(route)

def onConnectClick():
	route = str(routeEntry.get())
	connect(route)

def getMessage():
	message = None
	message = str(msgEntry.get())
...
#Second frame for the message input
